...
namespace assets {
    ...
    texture const& asset_manager::wall_tx_by_id(asset_pack::texture_id const id) const {
        return asset_packs[cur_set].wall_tx_by_id(id);
    }

    texture const& asset_manager::sprite_tx_by_id(asset_pack::texture_id const id) const {
        return asset_packs[cur_set].sprite_tx_by_id(id);
    }

    texture const& asset_manager::flat_tx_by_id(asset_pack::texture_id const id) const {
        return asset_packs[cur_set].flat_tx_by_id(id);
    }

    texture const& asset_manager::ui_tx_by_id(asset_pack::texture_id const id) const {
        return asset_packs[cur_set].ui_tx_by_id(id);
    }

    menu &asset_manager::menu_by_id(asset_pack::menu_id const id) {
        return asset_packs[cur_set].menu_by_id(id);
    }

    int asset_manager::display_menu(
