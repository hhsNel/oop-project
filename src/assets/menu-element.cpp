...
namespace assets {
	...
	menu_element::menu_element(int f, float bx, float by, float ex, float ey, texture const *b, std::string s, int cw, int ch, std::uint8_t r, std::uint8_t g, std::uint8_t b_col) :
		function(f),
		begin_x(bx),
		begin_y(by),
		end_x(ex),
	...
	menu_element menu_element::load_from_bin(util::resource const &res, std::vector<texture> const &ui_tx) {
		...
		return menu_element(f, bx, by, ex, ey, bg_tx, std::move(tmpl), cw, ch, static_cast<std::uint8_t>(r), static_cast<std::uint8_t>(g), static_cast<std::uint8_t>(b));
	}
	...
	void menu_element::selective_fmt(std::string_view key_filter, std::function<std::string(std::string_view)> fn) {
		if (key_filter.empty() || fmt_template.find(key_filter) != std::string::npos) {
			formatter = fn;
		}
	}

	int menu_element::render(rendering::renderer_2d const &r2d, int sw, int sh, input::mouse_state const &ms) const {
