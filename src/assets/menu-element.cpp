...
namespace assets {
	...
	menu_element::menu_element(int f, float const bx, float const by, float const ex, float const ey, texture const *const b, std::string const s)
		: function(f), begin_x(bx), begin_y(by), end_x(ex), end_y(ey), bg(b), str(s) {};
	...
	menu_element const menu_element::load_from_bin(util::resource const &res, std::vector<texture> const &ui_tx) {
		...
		return menu_element(f, bx, by, ex, ey, bg_tx, s);
	}
}
