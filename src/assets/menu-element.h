...
namespace assets {
	class menu_element {
		menu_element(int f, float bx, float by, float ex, float ey, texture const *b, std::string s, int cw, int ch, std::uint8_t r, std::uint8_t g, std::uint8_t b_col);
		...
		int render(rendering::renderer_2d const &r2d, int sw, int sh, input::mouse_state const &ms) const;

	public:
		static menu_element load_from_bin(util::resource const &res, std::vector<texture> const &ui_tx);
