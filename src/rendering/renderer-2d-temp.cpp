#include "renderer-2d-temp.h"

namespace rendering {

    renderer_2d_temp::renderer_2d_temp() : target(nullptr), tex_manager(nullptr), font_texture(nullptr) {}

    void renderer_2d_temp::set_target(rendering_backend *const tgt) {
        target = tgt;
    }

    void renderer_2d_temp::set_texture_manager(graphics::texture_manager const *const tm) {
        tex_manager = tm;
    }

    void renderer_2d_temp::set_font_texture(graphics::texture const *const font_tex) {
        font_texture = font_tex;
    }

    void renderer_2d_temp::draw_texture(graphics::texture const& tex, int x, int y, int w, int h) {
    ...
    void renderer_2d_temp::draw_rect(int x, int y, int w, int h, std::uint32_t color) {
