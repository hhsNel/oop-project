...
int main() {
    ...
    while (true) {
        ...
        auto world_to_screen = [&](float wx, float wy) -> std::pair<int, int> {
            return {
                static_cast<int>(CX + (wx - p.position.x) * SCALE),
                static_cast<int>(CY + (wy - p.position.y) * SCALE)
            };
        };
        ...
        // World grid — lines every 1 unit, thicker every 5 units
        {
            ...
            int grid_x0 = static_cast<int>(std::floor(p.position.x - visible_units_x));
            int grid_x1 = static_cast<int>(std::ceil (p.position.x + visible_units_x));
            int grid_y0 = static_cast<int>(std::floor(p.position.y - visible_units_y));
            int grid_y1 = static_cast<int>(std::ceil (p.position.y + visible_units_y));

            for (int gx = grid_x0; gx <= grid_x1; ++gx) {
                int sx     = static_cast<int>(CX + (gx - p.position.x) * SCALE);
                int sy_top = static_cast<int>(CY + (grid_y0 - p.position.y) * SCALE);
                int sy_bot = static_cast<int>(CY + (grid_y1 - p.position.y) * SCALE);
            ...
            for (int gy = grid_y0; gy <= grid_y1; ++gy) {
                int sx_l = static_cast<int>(CX + (grid_x0 - p.position.x) * SCALE);
                int sx_r = static_cast<int>(CX + (grid_x1 - p.position.x) * SCALE);
                int sy   = static_cast<int>(CY + (gy    - p.position.y) * SCALE);
        ...
        r2d.draw_text("POS:  x=" + fmt2(p.position.x) + "  y=" + fmt2(p.position.y),
