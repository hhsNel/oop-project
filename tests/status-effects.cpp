...
int main() {
	...
	while (std::cin >> cmd) {
		...
		if (cmd == "burning_tick") {
			...
			combat::burning b(10.0f, 5); // duration=10s, intensity=5
			b.affect(a);
		...
		// burning_via_actor — efekt nałożony przez add_effect tickuje przez update()
			...
			test_actor a(100.0f, 0.0f, 1.0f);
			a.add_effect(std::make_unique<combat::burning>(10.0f, 3));
		...
		// burning_multi_tick — kilka ticków w czasie
			...
			test_actor a(100.0f, 0.0f, 1.0f);
			a.add_effect(std::make_unique<combat::burning>(10.0f, 4));
		...
		// slowed_apply — on_apply redukuje prędkość o intensity%
			...
			combat::slowed s(5.0f, 50); // 50% spowolnienia
			s.on_apply(a);
		...
		// slowed_minimum — prędkość nie spada poniżej 10% bazy
			...
			test_actor a(100.0f, 0.0f, 100.0f);
			combat::slowed s(5.0f, 99); // 99% spowolnienia -> min 10%
			s.on_apply(a);
		...
		// slowed_via_actor — efekt nałożony przez add_effect, wygasa po czasie
			...
			test_actor a(100.0f, 0.0f, 100.0f);
			a.add_effect(std::make_unique<combat::slowed>(1.0f, 50));
		...
		// charmed_via_actor — efekt nałożony przez add_effect, wygasa po czasie
			...
			test_actor a(100.0f, 0.0f, 1.0f);
			a.add_effect(std::make_unique<combat::charmed>(1.0f, 1));
		...
		// Sprawdza że burning omija pancerz: armor bez zmian, hp spada o intensity
			...
			combat::burning b(10.0f, intensity);
			b.affect(a);
		...
		// Sprawdza wynik n ticków (co 0.5s)
			...
			a.add_effect(std::make_unique<combat::burning>(dur, intensity));
			for (int i = 0; i < n; ++i)
				a.update(0.5f);
		...
		// Sprawdza prędkość po on_apply
			...
			combat::slowed s(10.0f, intensity);
			s.on_apply(a);
